...
bool SamplingRateConverter::Init(SoundSourceL* _source, int _buffersize, ulong outrate)
{
	...
	MakeFilter(outrate);
	// FIR history [0..2*M] are zeros; audio is written from write onward.
	read = 0;
	write = 2 * M + 1;
	oo = 0;
	return true;
...
int SamplingRateConverter::FillMain(int samples)
{
	...
	// Only drop buffered audio when explicitly refilling on underrun (Mix path).
	// Skipping read with fillwhenempty==false caused periodic clicks on Linux/SDL.
