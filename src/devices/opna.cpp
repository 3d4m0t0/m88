...
bool OPNA::LoadRhythmSample(const char* path)
{
	...
	for (i=0; i<6; i++)
	{
		...
		if (file.Read(&fmt_chunk_size, 4) != 4) {
			break;
		}
		if (file.Read(&tag, 2) != 2 || file.Read(&nch, 2) != 2 ||
		    file.Read(&rate, 4) != 4 || file.Read(&avgbytes, 4) != 4 ||
		    file.Read(&align, 2) != 2 || file.Read(&bps, 2) != 2) {
			break;
		}
		if (fmt_chunk_size > 16) {
			uint16 ext_size = 0;
			if (file.Read(&ext_size, 2) != 2) {
				break;
			}
		}
		file.Seek(0x10 + 4 + static_cast<int32>(fmt_chunk_size), FileIO::begin);

		uint8 subchunkname[4];
		uint32 data_bytes = 0;
		bool found_data = false;
		for (int guard = 0; guard < 64; ++guard) {
			if (file.Read(subchunkname, 4) != 4) {
				break;
			}
			if (file.Read(&data_bytes, 4) != 4) {
				break;
			}
			if (memcmp(subchunkname, "data", 4) == 0) {
				found_data = true;
				break;
			}
			if (data_bytes > 0x1000000u) {
				break;
			}
			if (!file.Seek(static_cast<int32>(data_bytes), FileIO::current)) {
				break;
			}
		}
		if (!found_data) {
